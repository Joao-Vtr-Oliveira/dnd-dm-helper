...
export class CompendiumCreatureAdapterService {
	...
	private specialAbilities(monster: CompendiumMonster): CreatureSpecialAbility[] {
		...
		return sections.flatMap((feature, index) => {
			const recharge = feature.name.match(/\(\s*Recharge\s+([1-6])(?:\s*[-\u2013]\s*([1-6]))?\s*\)/i);
	...
	private describeSpellLevels(
		...
		return [
			...
			.map(([level, data]) => `${level}: ${(data.spells ?? []).map((spell) => this.renderer.renderText(spell)).join(', ')}`)
			.filter((line) => !line.endsWith(': ')),
			...Object.entries(spellLists).map(
				([list, values]) => `${list}: ${values.map((spell) => this.renderer.renderText(spell)).join(', ')}`,
			),
		]
			.join('\n');
	}
